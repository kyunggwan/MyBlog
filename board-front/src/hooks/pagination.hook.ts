...
const usePagination = <T>(countPerPage: number) => {
  ...
  const setView = () => {
    ...
    const LAST_INDEX =
      totalPageList.length > countPerPage * currentPage
  ...
  //        effect: total list가 변경될 떄마다 실행할 작업        //
  useEffect(() => {
  ...
  //        effect: current page가 변경될 떄마다 실행할 작업        //
  useEffect(setView, [currentPage]);
  //        effect: current section가 변경될 떄마다 실행할 작업        //
